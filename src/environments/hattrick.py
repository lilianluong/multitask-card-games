...
class HatTrick(TrickTakingGame):
	...
	for every trick, which can be won by playing the highest of the suit that was led or a trump card. 
	A trump card can be played at any time and the hierarchy for the trump suit is 5, J, Highest Card of Hearts, 
	followed by the remaining cards in the trump suit in the traditional numerical order. Winning three
	tricks leads to an additional bonus of 25 points, but crossing three tricks results in a 50 point penalty.
	...
	def _end_trick(self) -> Tuple[List[int], int]:
		...
		if self.scores[winning_player] == 10:
			rewards[winning_player] = 30 #25pt bonus
		elif self.scores[winning_player] == 40: #aka crossing three tricks
			rewards[winning_player] = -10
		else:	
		...
		for player_index, card in enumerate(played_cards):
			...
			#special trump cases
				if (card.value == 5) or (card.value == 11 and winning_card.value != 5) or \
					(card == Card(Suit.HEARTS, self._num_cards-1) and winning_card.value not in {5, 11}): 
				...
				(card.suit == trump_suit and winning_card.suit != trump_suit) or (card == Card(Suit.HEARTS, self._num_cards-1))): 
				winning_index = player_index
				winning_card = card
